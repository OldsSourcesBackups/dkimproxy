...
sub init
{
	...
	if (defined $self->{KeyFile})
	{
		...
		$self->{private} = Mail::DomainKeys::Key::Private->load(
				File => $self->{KeyFile});
	}
...
sub finish_header
{
	...
	my $canon_class = $self->get_canonicalization_class($self->{"Method"});
